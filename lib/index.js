 ...
 *
 */
class TrackComponent extends TrackDSLExtendable {
  /**
   * Initialize
  ...
  _watchAttrs() {
    if (this.onattrschanged) {
      ...
      const previous = this.vnode.state.component.attrs;

      if (!ObjectHelper.deepEqual(current, previous)) {
        this.onattrschanged(current, previous);
        this.vnode.state.component.attrs = ObjectHelper.deepMerge({}, current);
      }
    }
  }
}
